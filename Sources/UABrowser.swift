...
public struct OperaBrowser: UABrowser {
    ...
    public func userAgentPlatformInfo(for device: UADevice) -> String {
        ...
        switch device {
        case is iOSDevice:
            return "AppleWebKit/605.1.15 (KHTML, like Gecko) Version/19.0 Mobile/15E148 Safari/604.1 OPT/\(platformVersion)"
        case is AndroidDevice:
            return "AppleWebKit/537.36 (KHTML, like Gecko) Chrome/\(chromeVersion) Mobile Safari/537.36 OPR/\(platformVersion).0.0"
        default:
            return "AppleWebKit/537.36 (KHTML, like Gecko) Chrome/\(chromeVersion) Safari/537.36 OPR/\(platformVersion).0.0"
        }
    }
}
